...
#
cor <- cor %>% 
  mutate(delta = casos-lag(casos),
         deltaPrev = lag(delta),
         growthFactor = delta/deltaPrev)

ggplot(cor %>% filter(dias>3),
       aes(x=dias,y=growthFactor)) + 
  geom_point() + 
  theme_bw() + 
  stat_smooth(method=lm,se=FALSE) +   
  labs(title = bquote("Growth Factor=" ~ Delta* N[t] / Delta* N[t-1] )) + 
  annotate(geom="text", x=5, y=8, label="Fuente @msalnacion\n by @larysar",color="red",size=2)
...
ggplot(cor1,aes(x=fecha,y=N,color=tipo)) + geom_point() + theme_bw() + scale_color_viridis_d() + scale_color_viridis_d() + ylab("Casos") + 
  geom_line(data=predexp, aes(x=fecha,y = pred,color=tipo), size = .5) 
